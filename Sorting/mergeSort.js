...
two sorted arrays and a comparator and returns a new sorted array. You implemented this function 
in the previous exercise, so copy and paste that code here.
...
function merge(arr1, arr2, compr){
    let result = []
    let i = 0;
    let j = 0
    
    while(i < arr1.length && j < arr2.length){
    // comparator funx is provided, use it otherwise treat as arithmetic comparison 
        if(typeof compr === 'function'){
            if(compr(arr1[i], arr2[j]) < 0){
                result.push(arr1[i])
                i++
            }else {
                result.push(arr2[j])
...
function mergeSort(arr, compr){
    // break this arr into n array and call using recursion
    // merge to compare them and return new array that is sorted
    
    if(arr.length <= 1) return arr;
    let mid = Math.floor(arr.length / 2)
    let left = mergeSort(arr.slice(0, mid), compr)
    let right = mergeSort(arr.slice(mid), compr)
    
    return merge(left, right, compr)
}
